    ...
    if (loading) return (
        <div className="flex items-center justify-center min-h-[60vh]">
            <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-surface-500 font-bold uppercase tracking-widest text-sm">Chargement de votre espace...</p>
            </div>
        </div>
    );
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-sm font-black uppercase tracking-[0.2em] text-surface-400">Espace Partenaire</span>
                        </div>
                        <h1 className="text-2xl font-black text-surface-900 tracking-tight leading-none">Bonjour, <span className="text-primary">{user?.name}</span></h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    ...
                    <motion.div 
                        ...
                        whileTap={{ scale: 0.98 }}
                        className="bg-card-bg p-7 rounded-3xl shadow-premium border border-outline-variant hover:border-primary/20 transition-all group transition-colors duration-300 cursor-pointer"
                    >
                        ...
                        <h3 className="text-3xl font-black text-surface-900 leading-none">{stat.value}</h3>
                    </motion.div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <motion.div variants={itemVariants} className="lg:col-span-2 bg-card-bg p-8 rounded-3xl shadow-premium border border-outline-variant flex flex-col transition-colors duration-300">
                    <div className="flex justify-between items-center mb-10">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-sm font-black text-surface-900 tracking-tight leading-none uppercase tracking-tighter">Indicateurs de Performance</h2>
                                <p className="text-xs text-surface-400 font-bold uppercase tracking-widest mt-1">Évolution de l'atteinte des cibles</p>
                            </div>
                        </div>
                    </div>
                ...
                <motion.div variants={itemVariants} className="bg-card-bg p-8 rounded-3xl shadow-premium border border-outline-variant flex flex-col transition-colors duration-300">
                    <div className="flex items-center gap-3 mb-8">
                        ...
                        <h2 className="text-sm font-black text-surface-900 tracking-tight leading-none uppercase tracking-tighter">Mes Dossiers</h2>
                    </div>
                    ...
                    <div className="flex-1 space-y-4 overflow-y-auto max-h-[400px] pr-2 custom-scrollbar">
                        {conventions.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-full py-12 text-center opacity-30">
                                ...
                                <p className="text-sm font-black uppercase tracking-widest text-surface-400">Aucun dossier actif</p>
                            </div>
                        ) : (
                            <AnimatePresence>
                                {conventions.map((conv, idx) => (
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-3">
                                            <h4 className="text-sm font-black text-surface-900 group-hover:text-primary transition-colors leading-snug">{conv.name}</h4>
                                        </div>
                                        
                                        <div className="flex flex-wrap gap-2 mb-4">
                                            <div className={`px-2 py-0.5 rounded-lg text-xs font-black flex items-center gap-1.5 border ${getStatusStyles(conv.status)} uppercase tracking-widest`}>
                                                {getStatusIcon(conv.status)}
                                                {conv.status.replace('_', ' ')}
                                            </div>
                                            {conv.kpis?.length > 0 && (
                                                <div className="px-2 py-0.5 rounded-lg text-xs font-black bg-surface-100 text-surface-500 border border-surface-200 uppercase tracking-widest">
                                                    {conv.kpis.length} KPIs
                                                </div>
                                            )}
                                        </div>

                                        <div className="flex items-center justify-between text-xs font-bold text-surface-400">
