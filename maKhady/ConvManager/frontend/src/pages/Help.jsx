...
const Help = () => {
    ...
    return (
        ...
        >
            {/* Header */}
            <div className="bg-[#2E2F7F] dark:bg-slate-800 p-16 rounded-[3.5rem] text-white relative overflow-hidden shadow-2xl shadow-[#2E2F7F]/20 dark:shadow-black/20">
                <div className="relative z-10 space-y-6">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-px bg-[#B68F40]"></div>
                        <span className="text-sm font-black uppercase tracking-[0.4em] text-[#B68F40]">Documentation Officielle</span>
                    </div>
                    <h1 className="text-4xl font-black tracking-tight leading-tight uppercase max-w-2xl">
                        CENTRE D'AIDE & GUIDE DES PROCÉDURES UIDT
                    </h1>
                    <p className="text-lg font-medium text-white/60 max-w-xl leading-relaxed">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                {sections.map((section, idx) => (
                    <div key={idx} className="bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] shadow-sm border border-gray-100 dark:border-slate-800 flex flex-col hover:shadow-2xl hover:shadow-black/5 dark:hover:shadow-black/20 transition-all group">
                        <div className="flex items-center gap-5 mb-8">
                            <div className="w-14 h-14 bg-gray-50 dark:bg-white/5 rounded-2xl flex items-center justify-center text-[#B68F40] group-hover:bg-[#2E2F7F] dark:group-hover:bg-indigo-600 group-hover:text-white transition-all transform group-hover:rotate-6 shadow-sm border border-gray-100 dark:border-white/10">
                                <span className="material-symbols-outlined text-[28px]">{section.icon}</span>
                            </div>
                            <h3 className="text-lg font-black text-[#2E2F7F] dark:text-white tracking-tight">{section.title}</h3>
                        </div>
                        <p className="text-base font-black text-slate-500 dark:text-slate-400 mb-8 leading-relaxed uppercase tracking-[0.2em]">{section.content}</p>
                        
                        <div className="space-y-6 flex-1">
                            {section.steps && section.steps.map((step, sidx) => (
                                <div key={sidx} className="flex gap-4 relative">
                                    {sidx < section.steps.length - 1 && <div className="absolute left-[9px] top-6 bottom-[-24px] w-[2px] bg-gray-50 dark:bg-slate-800"></div>}
                                    <div className="w-5 h-5 rounded-full border-2 border-[#B68F40] bg-white dark:bg-slate-900 z-10 flex items-center justify-center text-[11px] font-black text-[#B68F40]">{sidx + 1}</div>
                                    <div className="space-y-1 pt-0.5">
                                        <h4 className="text-base font-black text-[#2E2F7F] dark:text-white uppercase tracking-wide">{step.label}</h4>
                                        <p className="text-base text-slate-600 dark:text-slate-400 font-bold leading-relaxed">{step.desc}</p>
                                    </div>
                                </div>
                            ))}

                            {section.items && section.items.map((item, iidx) => (
                                <div key={iidx} className="bg-gray-50/50 dark:bg-white/5 p-4 rounded-xl border border-gray-50 dark:border-white/10">
                                    <h4 className="text-base font-black text-[#2E2F7F] dark:text-white uppercase tracking-wide mb-1 flex items-center gap-2">
                                        <div className="w-1.5 h-1.5 rounded-full bg-[#B68F40]"></div>
                                        {item.label}
                                    </h4>
                                    <p className="text-base text-slate-600 dark:text-slate-400 font-bold leading-relaxed">{item.desc}</p>
                                </div>
                            ))}

                            {section.roles && section.roles.map((role, ridx) => (
                                <div key={ridx} className="flex gap-4 items-start py-2">
                                    <span className="material-symbols-outlined text-[18px] text-gray-300 dark:text-slate-600">verified_user</span>
                                    <div className="space-y-1">
                                        <h4 className="text-base font-black text-[#2E2F7F] dark:text-white uppercase tracking-wide">{role.name}</h4>
                                        <p className="text-base text-slate-600 dark:text-slate-400 font-bold leading-relaxed">{role.desc}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            <div className="bg-gradient-to-r from-white dark:from-slate-900 to-gray-50 dark:to-slate-800 p-10 rounded-[2.5rem] border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden relative group">
                <div className="absolute right-0 top-0 w-1/3 h-full bg-indigo-50/30 dark:bg-indigo-900/10 -skew-x-12 translate-x-1/2"></div>
                <div className="relative z-10 grid grid-cols-1 lg:grid-cols-4 gap-10 items-center">
                    <div className="lg:col-span-1 flex flex-col items-center lg:items-start text-center lg:text-left gap-4">
                        <div className="w-16 h-16 bg-[#2E2F7F] dark:bg-indigo-600 text-white rounded-2xl flex items-center justify-center shadow-xl">
                            <span className="material-symbols-outlined text-[32px]">notifications_active</span>
                        </div>
                        <h3 className="text-lg font-black text-[#2E2F7F] dark:text-white uppercase tracking-tight">Système de<br/>Notification</h3>
                    </div>
                        ...
                        {[
                            ...
                            <div key={i} className="space-y-3">
                                <div className="flex items-center gap-3">
                                    <span className="material-symbols-outlined text-[#B68F40] text-base">{item.icon}</span>
                                    <h4 className="text-base font-black text-[#2E2F7F] dark:text-white uppercase tracking-widest">{item.title}</h4>
                                </div>
                                <p className="text-base text-slate-600 dark:text-slate-400 font-bold leading-relaxed">{item.desc}</p>
                            </div>
                        ))}
            ...
            {/* Assistance Section */}
            <div className="bg-white dark:bg-slate-900 p-12 rounded-[3.5rem] border border-gray-100 dark:border-slate-800 flex flex-col md:flex-row items-center justify-between gap-10 shadow-sm">
                <div className="flex items-center gap-8">
                    <div className="w-20 h-20 bg-green-50 dark:bg-emerald-900/20 rounded-[2rem] flex items-center justify-center text-green-500 dark:text-emerald-400 border border-green-100 dark:border-emerald-900/30">
                        <span className="material-symbols-outlined text-[40px]">support_agent</span>
                    </div>
                    <div className="space-y-2">
                        <h3 className="text-xl font-black text-[#2E2F7F] dark:text-white">Besoin d'une assistance technique ?</h3>
                        <p className="text-base font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest leading-relaxed">
                ...
                <button 
                    ...
                    className="px-12 py-5 bg-[#2E2F7F] dark:bg-indigo-600 text-white rounded-2xl text-sm font-black uppercase tracking-[0.2em] shadow-xl shadow-[#2E2F7F]/20 dark:shadow-indigo-500/20 hover:scale-105 active:scale-95 transition-all"
                >
                    {t('ouvrir_ticket')}
                </button>
            </div>
